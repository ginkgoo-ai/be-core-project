...
public class ProjectWriteServiceImpl implements ProjectWriteService {
	...
	public ProjectResponse createProject(ProjectCreateRequest request, String workspaceId, String userId) {
		...
		if (request.getRoles() != null && !request.getRoles().isEmpty()) {
			request.getRoles().forEach(roleRequest -> {
				...
				activityLogger.log(workspaceId, savedProject.getId(), null, ActivityType.ROLE_CREATED,
						Map.of("user", userId, "project", savedProject.getName(), "roleName", role.getName()), null,
						userId);
			});
		...
		activityLogger.log(workspaceId, savedProject.getId(), null, ActivityType.PROJECT_CREATED,
				Map.of("user", userId, "projectName", savedProject.getName()), null, userId);
	...
	public Project updateProject(String projectId, ProjectUpdateRequest request, String workspaceId) {
		Project project = projectRepository.findByIdAndWorkspaceId(projectId, workspaceId)
			.orElseThrow(
					() -> new ResourceNotFoundException("Project", "id&workspaceId", projectId + ":" + workspaceId));
	...
	public Project updateProjectStatus(String id, ProjectStatus status) {
		...
		activityLogger.log(project.getWorkspaceId(), project.getId(), null, ActivityType.PROJECT_STATUS_CHANGE,
				Map.of("project", updatedProject.getName(), "previousStatus", project.getStatus().getValue(),
						"newStatus", status, "newStatus", updatedProject.getStatus().getValue()),
	...
	public ProjectRoleResponse createRole(String projectId, ProjectRoleRequest request) {
		...
		if (!ObjectUtils.isEmpty(savedRole.getSides())) {
			roleSideFilesMap = storageClient.getFileDetails(Arrays.stream(savedRole.getSides()).toList())
				.getBody()
				.stream()
