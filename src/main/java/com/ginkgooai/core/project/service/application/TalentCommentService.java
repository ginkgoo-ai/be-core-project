...
public class TalentCommentService {
     ...
     */
    @Transactional
    public TalentCommentResponse addComment(String workspaceId, String talentId, String userId, TalentCommentRequest request) {
        ...
        TalentComment comment = TalentComment.builder()
        ...
        UserInfoResponse userInfo = identityClient.getUserById(userId).getBody();

        return TalentCommentResponse.from(savedComment, userInfo);
    }
    ...
    public List<TalentCommentResponse> getComments(String workspaceId, String talentId) {
        ...
        List<TalentComment> comments = talentCommentRepository.findByWorkspaceIdAndTalentId(workspaceId, talentId, sort);

        // Get user info for all comment creators
        List<String> userIds = comments.stream()
            .map(TalentComment::getCreatedBy)
            .distinct()
        ...
        return comments.stream()
            .map(comment -> TalentCommentResponse.from(comment, userInfoMap.get(comment.getCreatedBy())))
            .collect(Collectors.toList());
    }
    ...
    public void deleteComment(String workspaceId, String commentId, String userId) {
        ...
        TalentComment comment = talentCommentRepository.findByWorkspaceIdAndId(workspaceId, commentId)
            .orElseThrow(() -> new ResourceNotFoundException("Comment", "workspaceId:id", String.join(" ", workspaceId, commentId)));
