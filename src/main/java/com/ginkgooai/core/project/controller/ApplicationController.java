...
public class ApplicationController {
	...
	public ResponseEntity<List<ApplicationResponse>> createApplications(
		...
		return ResponseEntity.ok(applicationService.createApplications(request, ContextUtils.getWorkspaceId(),
	...
	public ResponseEntity<Page<ApplicationResponse>> listApplications(
			...
			@Parameter(description = "End date for submission creation (format: yyyy-MM-dd'T'HH:mm:ss)") @RequestParam(
					required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endDateTime,
			@Parameter(description = "View Mode") @RequestParam(defaultValue = "readingList") String viewMode,
			@Parameter(description = "Search keyword for talent name or email or role name") @RequestParam(
		...
		return ResponseEntity
			.ok(applicationService.listApplications(ContextUtils.getWorkspaceId(), ContextUtils.getUserId(), projectId,
					roleId, talentId, startDateTime, endDateTime, viewMode, keyword, status, pageable));
	}
	...
	public ResponseEntity<List<ApplicationCommentResponse>> addComment(
		...
		return ResponseEntity
			.ok(applicationService.addComment(ContextUtils.getWorkspaceId(), id, ContextUtils.getUserId(), content));
	}
	...
	public ResponseEntity<List<ApplicationNoteResponse>> addNote(
		...
		return ResponseEntity.ok(applicationService.addNote(ContextUtils.getWorkspaceId(), id, ContextUtils.getUserId(),
				request.getContent()));
	}
	...
	public ResponseEntity<SubmissionResponse> createSubmission(
		...
		return ResponseEntity
			.ok(submissionService.createSubmission(ContextUtils.getWorkspaceId(), request, ContextUtils.getUserId()));
	}

	@Operation(summary = "Update application status",
			description = "Updates the status of an application and optionally adds a comment")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Application status updated successfully"),
			@ApiResponse(responseCode = "404", description = "Application not found"),
	...
	public ResponseEntity<ApplicationResponse> updateApplicationStatus(
		...
		ApplicationResponse response = applicationService.updateApplicationStatus(applicationId,
				ContextUtils.getWorkspaceId(), request);
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "204", description = "Submission successfully deleted"),
	...
	@Operation(summary = "Guest(Talent) Add comment to submission",
	...
	public ResponseEntity<ApplicationStatisticsResponse> getApplicationStatusCountsByProject(
			...
			@Parameter(description = "Role ID filter") @RequestParam(required = false) String roleId) {
		ApplicationStatisticsResponse response = applicationService.getApplicationStatusCountsByProject(projectId,
				roleId);
